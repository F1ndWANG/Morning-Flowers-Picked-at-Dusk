...
def build_creative_diagnosis(creative: dict, campaign: dict, case_context: dict) -> dict:
  ...
  _add_if(recommendations, breakdown.get("noveltyPenalty", 0) > 0.03, "该方案与已选高分方案相似，可尝试换场景或换卖点角度。")
  ...
  health_score = max(0, min(1, (
    features.get("commercialQuality", 0.5) * 0.35
    + alignment.get("overallAlignment", 0.3) * 0.25
    + metrics.get("confidence", 0.6) * 0.20
    + compliance.get("scoreFactor", 0.9) * 0.20
  )))
